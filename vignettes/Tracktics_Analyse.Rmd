...
data_single <- do.call("cbind", lapply(1:(length(tracktics_files) - 1), function(i) tracktics_read_player_stats(file = tracktics_files[i], 
                                                                                                                player_first_name = first_name, 
                                                                                                                player_last_name = last_name,
                                                                                                                match_name = red_match_names[[i]])))
...
# For every match add 100 in width
png(file = paste0(full_path, "/Einzelspieler/", Sys.Date(), "_",first_name,"_",last_name,".png"), height = 470, width = 320 + (ncol(df_data_single_tt)-1) * 75)
gridExtra::grid.table(df_data_single_tt, rows = NULL)
dev.off()
